      ...
      <div className="relative z-10 flex font-sans flex-col items-center justify-center min-h-screen">
        <div className="bg-white/50 font-sans py-3 px-20 flex flex-col justify-center items-center gap-3 rounded-2xl">
              ...
              <label key={value} className="cursor-pointer">
                <input
                  ...
                  onChange={() => {
                    ...
                    setPlaceholder(value === "Estudante" ? "Email" : "CNPJ");
                  }}
                />
                <span className={`font-heading inline-block py-2 px-5 text-zinc-500 peer-checked:bg-blue-700 peer-checked:text-white ${["Estudante", "Escola"].includes(value) ? "border-r" : ""}`}>
                  {value}
                </span>
              </label>
            ))}
          ...
          <div className="w-fit flex flex-col justify-center items-center gap-3">
            ...
            <div className="flex flex-row gap-2 items-center bg-white px-1.5 rounded-full">
              <RiLockLine size={30} className="text-zinc-500 border rounded-full p-0.5" />
              <input type="password" placeholder="Senha" className="bg-white p-2 rounded-full outline-0" />
            </div>
            <Link href={`/${userType}/home`} className="w-full">
              <Button text="Entrar" />
            </Link>
          </div>

          <div className="flex flex-row w-full justify-between p-2 overflow-hidden">
            <Link href={`/formulario/${userType}`} className="text-blue-600 border-b font-sans border-transparent cursor-pointer hover:border-blue-600">
              Criar conta
            </Link>
            <Link href="/recuperar-senha" className="text-blue-600 border-b border-transparent font-sans cursor-pointer hover:border-blue-600">
              Esqueci a senha
            </Link>
          </div>
        </div>
